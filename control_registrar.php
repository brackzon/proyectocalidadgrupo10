...
$array_atributos2 = [];
   ...
   //SE ESTA CREANDO UN ARRAT DE ATRIBUTIS
    ...
    fwrite($archivo,'$sql = "INSERT INTO '.$tabla_sql.' ('.$string_atributos.')'."\n");
    fwrite($archivo,'VALUES ('.$string_atributos2.');";'."\n");
...
fwrite($archivo,"<?php"."\n");
fwrite($archivo,'$lista = $_REQUEST[\'lista\'];'."\n");
fwrite($archivo,'$sql = "SELECT id_foraneo FROM foranio WHERE atributoForaneo = $lista";'."\n");
fwrite($archivo,'           $resultado = $mysqli->query($sql);'."\n");
fwrite($archivo,''."\n");
fwrite($archivo,'while($fila = $resultado->fetch_assoc()) {'."\n");
    fwrite($archivo,'$id_for = $fila[\'id_fpraneo\'];'."\n");
    fwrite($archivo,' }'."\n");
    fwrite($archivo,'if($id_for)...'."\n");
fwrite($archivo,'?>'."\n");